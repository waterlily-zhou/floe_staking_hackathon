...
import { readStakingPosition } from "./readPositions"; // Import from readPositions.ts
dotenv.config();
...
// Define the position data structure to match readStakingPosition return type
...
// Define token information structure for better documentation
interface TokenInfo {
  address: string;
  symbol: string;
  name: string;
 ...
 */
export function saveAutoClaimSettings(settings: Partial<AutoClaimSettings>): void {
  try {
    const dataDir = path.dirname(AUTO_CLAIM_SETTINGS_FILE);
    if (!fs.existsSync(dataDir)) {
      fs.mkdirSync(dataDir, { recursive: true });
...
export async function claimRewards(
  ...
  currentGasPriceGwei = parseFloat(ethers.formatUnits(gasPrice.gasPrice || 0n, "gwei"));
  ...
  try {
    ...
    try {
      ...
      if (userAddress) {
        positionData = await readStakingPosition(userAddress) as PositionData;
    ...
    try {
      const gauge = new ethers.Contract(GAUGE_ADDRESS, GAUGE_ABI, wallet);
      ...
      console.log(`   Found ${ethers.formatEther(balance)} LP tokens in gauge`);
      
      // Get all claimable rewards before claiming
      const gaugeRewards = [];
      
      // Check BAL rewards
      const balToken = new ethers.Contract(BAL_TOKEN, ERC20_ABI, provider);
      ...
      try {
        ...
        for (let i = 0; i < Number(rewardCount); i++) {
          try {
            ...
            if (claimableAmount > 0n) {
              const formattedAmount = ethers.formatUnits(claimableAmount, decimals);
        ...
        try {
          ...
          if (claimableBAL > 0n) {
            const formattedAmount = ethers.formatUnits(claimableBAL, balDecimals);
      ...
      for (const reward of gaugeRewards) {
        ...
        console.log(`     Claimed ${ethers.formatUnits(reward.amount, reward.decimals)} ${reward.symbol} ($${valueUSD.toFixed(2)})`);
      }
 ...
 */
function saveClaimLog(log: any): void {
  try {
 ...
 */
function calculateUSDValue(tokenAddress: string, amount: bigint, decimals: number): number {
  ...
  return Number(ethers.formatUnits(amount, decimals)) * tokenPriceUSD;
}

/**
 * Claim rewards from specific gauges
 */
export async function claimRewardsFromGauges(
  ...
  try {
    ...
    if (useDelegation && delegationContractAddress) {
      ...
      const txHash = await executeClaimWithDelegation(
        walletAddress,
        gaugeAddresses,
    ...
    for (const gaugeAddress of gaugeAddresses) {
      try {
        ...
        const gaugeContract = new ethers.Contract(gaugeAddress, gaugeABI, userWallet);
