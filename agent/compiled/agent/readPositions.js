 ...
 */
async function getTokenPrice(tokenAddress, tokenSymbol) {
    // Check cache first
    ...
    try {
        ...
        const response = await axios_1.default.get(`https://api.coingecko.com/api/v3/simple/token_price/ethereum?contract_addresses=${tokenAddress}&vs_currencies=usd`);
 ...
 */
async function getBPTPrice(bptAddress, bptSymbol) {
    try {
 ...
 */
async function readStakingPosition(walletAddress) {
    ...
    try {
        ...
        for (const gaugeInfo of exports.COMMON_GAUGES) {
            try {
                console.log(`\nChecking gauge: ${gaugeInfo.name} (${gaugeInfo.gauge})`);
                // Connect to contracts
                const bptToken = new ethers_1.ethers.Contract(gaugeInfo.pool, ERC20_ABI, provider);
                const gauge = new ethers_1.ethers.Contract(gaugeInfo.gauge, GAUGE_ABI, provider);
                // Get token info
                ...
                if (stakedBalance > 0n) {
                    ...
                    try {
                        ...
                        if (claimableBAL > 0n) {
                            const balToken = new ethers_1.ethers.Contract(BAL_TOKEN_ADDRESS, ERC20_ABI, provider);
                        ...
                        if (rewardCount > 0) {
                            for (let i = 0; i < Number(rewardCount); i++) {
                                try {
                                    ...
                                    const rewardToken = new ethers_1.ethers.Contract(rewardTokenAddress, ERC20_ABI, provider);
 ...
 */
async function readMultiplePositions(addresses) {
    console.log(`\n${"=".repeat(60)}`);
 ...
 */
async function getPositionsForAddress(address) {
    ...
    try {
        ...
        for (const gauge of allGauges) {
            try {
                const gaugeContract = new ethers_1.ethers.Contract(gauge.address, gaugeABI, provider);
                ...
                if (balance > 0n) {
                    ...
                    try {
                        if (gauge.poolAddress) {
                            const poolToken = new ethers_1.ethers.Contract(gauge.poolAddress, poolTokenABI, provider);
...
async function GET(request) {
    try {
        ...
        try {
            ...
            const positionFile = path_1.default.join(dataDir, 'position.json');
            ...
            if (!refresh && fs.existsSync(positionFile)) {
                ...
                return server_1.NextResponse.json(positionData);
            }
            ...
            return server_1.NextResponse.json({ error: 'Failed to fetch position data' }, { status: 500 });
        }
    }
    catch (error) {
...
//# sourceMappingURL=readPositions.js.map